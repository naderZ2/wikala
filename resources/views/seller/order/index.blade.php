...
@if(Route::currentRouteName() == 'seller.order.under_preparation')

<h3> @lang('lang.Under_Preparation_Orders') </h3>

@elseif(Route::currentRouteName() == 'seller.order.completed')

<h3> @lang('lang.Completed_Orders') </h3>

@elseif(Route::currentRouteName() == 'seller.order.new')
							...
							<tbody id="userTable">
								@forelse ($orders as $order)
									<tr>
										...
										<td>
											<a href="{{ route('seller.order.details',$order->id) }}" class="btn btn-info m-1 " >@lang('lang.details')</a>


											@if ( $order->status !=='delivered' && $order->status !=='cancel')

											{{-- @can('edit order status') --}}
											<a href="{{ route('seller.order.change_status',[$order->id,'normal']) }}" class="btn btn-primary m-1" >@lang('lang.change_status')</a>
											{{-- @endcan	 --}}
											{{-- @can('cancel orders') --}}
											<a href="{{ route('seller.order.change_status',[$order->id,'cancel']) }}" class="btn btn-danger m-1" >@lang('lang.cancel')</a>
											{{-- @endcan	 --}}
											@endif
