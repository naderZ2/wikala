...
class OrderController extends Controller
{
    ...
    public function generateInvoice(Request $request, $id)
    {
        ...
        $order = Order::where('id', $id)
            ->where('seller_id', $request->user()->getMainSellerId())
            ->with('user:id,name,phone,country_code,email', 'orderDetails.product', 'orderDetails.variation.attributes.attribute', 'seller:id,email,name', 'address', 'address.region')
            ->firstOrFail();
        ...
        if ($order->user && $order->user->phone) {
            try {
                ...
                Log::warning('Arrive Whats seller invoice delivery failed.', [
                    'order_id' => $order->id,
                ]);
            }
        }
